...
--  Q Q-13. Write an SQL query to print details of the Workers whose SALARY lies between 100000 and 500000.
...
select * from Bonus where mod(WORKER_REF_ID,2)<>0;
...
-- for Title table
--  Here we can see that the rows are not ordered

select * from (select * from Title order by WORKER_REF_ID) where mod(WORKER_REF_ID,2)<>0;
...
-- Q.Q-23. Write an SQL query to show the top n (say 10) records of a table.
...
select FIRST_NAME, LAST_NAME, DEPARTMENT 
 ...
 select min(SALARY) from Worker order by SALARY desc limit 4,1;
